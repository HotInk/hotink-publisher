...
Overlay = Class.create({
	...
	render: function( new_content ){
		if (this.start()){
			...
			this.overlay_content.insert( 
				Builder.node('div', { className: 'small_top_button clearfix'},
					Builder.node('a', { href:"#", className:'close_window', onclick:'page_overlay.end();return false;' },
						Builder.node('img', { src:"/images/window_x.png" })
					)
				) 
			);
